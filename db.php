...
function init_db(): void {
    ...
    // Seed only if empty
    $count = $db->query("SELECT COUNT(*) FROM meta")->fetchColumn();
    if ($count > 0) return;
...
function seed_db(PDO $db): void {
    ...
    $stmt = $db->prepare("INSERT INTO meta (key, value) VALUES (:k, :v)");
    foreach ($meta as $k => $v) {
    ...
    // --- BREADCRUMBS ---
    $breadcrumbs = [
        ['Türk İfşa', 'https://www.indiedevday.es/'],
        ['Türk İfsa', 'https://www.indiedevday.es/'],
        ['Türk İfşa İzle', 'https://www.indiedevday.es/'],
        ['Türk Liseli İfşa', 'https://www.indiedevday.es/'],
        ['Türk Ünlü İfşa', 'https://www.indiedevday.es/'],
        ['Fenomen İfşa', 'https://www.indiedevday.es/'],
        ['Ünlü İfşa', 'https://www.indiedevday.es/'],
        ['Liseli İfşa', 'https://www.indiedevday.es/'],
        ['Amatör Porn', 'https://www.indiedevday.es/'],
        ['Türk İfsa Twitter', 'https://www.indiedevday.es/'],
        ['Telegram İfsa', 'https://www.indiedevday.es/'],
        ['Türk İfsa Telegram', 'https://www.indiedevday.es/'],
        ['İfsa Telegram', 'https://www.indiedevday.es/'],
        ['Türk İfşa Videoları', 'https://www.indiedevday.es/'],
        ['Türk İfşa İçerikleri', 'https://www.indiedevday.es/'],
        ['Türk İfşa Platformları', 'https://www.indiedevday.es/'],
        ['Türk İfşa Siteleri', 'https://www.indiedevday.es/'],
        ['Türk İfşa Telegram', 'https://www.indiedevday.es/'],
        ['Telegram İfşa', 'https://www.indiedevday.es/'],
        ['İfşa Telegram', 'https://www.indiedevday.es/'],
        ['Türk İfşa Twitter', 'https://www.indiedevday.es/'],
        ['Türk Porno', 'https://www.indiedevday.es/'],
        ['Türk Porno İzle', 'https://www.indiedevday.es/'],
    ];
    $stmt = $db->prepare("INSERT INTO breadcrumbs (text, href, sort_order) VALUES (:t, :h, :s)");
    foreach ($breadcrumbs as $i => [$t, $h]) {
    ...
    $stmt = $db->prepare("INSERT INTO sections (heading, body, sort_order) VALUES (:h, :b, :s)");
    foreach ($sections as $i => [$h, $b]) {
        $stmt->execute([':h' => $h, ':b' => $b, ':s' => $i]);
    }
    ...
    $stmt = $db->prepare("INSERT INTO table1_rows (col_type, col_features, col_access, sort_order) VALUES (:t,:f,:a,:s)");
    foreach ($t1 as $i => [$t, $f, $a]) {
        $stmt->execute([':t' => $t, ':f' => $f, ':a' => $a, ':s' => $i]);
    }
    ...
    $stmt = $db->prepare("INSERT INTO table2_rows (col_type, col_features, col_duration, sort_order) VALUES (:t,:f,:d,:s)");
    foreach ($t2 as $i => [$t, $f, $d]) {
        $stmt->execute([':t' => $t, ':f' => $f, ':d' => $d, ':s' => $i]);
    }
    ...
    $stmt = $db->prepare("INSERT INTO tips (strong_text, body, sort_order) VALUES (:s,:b,:o)");
    foreach ($tips as $i => [$s, $b]) {
        $stmt->execute([':s' => $s, ':b' => $b, ':o' => $i]);
    }
